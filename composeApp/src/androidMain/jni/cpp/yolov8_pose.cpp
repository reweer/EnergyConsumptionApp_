...
Java_com_jetbrains_kmpapp_YoloV8Ncnn_loadModel(JNIEnv* env, jobject obj, jobject assetManager, jint modelId, jint cpuGpu) {
    ...
    int modelShape = 640;
    bool useGpu = (cpuGpu == 1);

    return inference->loadNcnnNetwork(mgr, modelType, modelShape, new float[3]{0.0f, 0.0f, 0.0f}, new float[3]{1/255.f, 1/255.f, 1/255.f}, useGpu) == 0 ? JNI_TRUE : JNI_FALSE;
}


extern "C" JNIEXPORT jobject JNICALL
Java_com_jetbrains_kmpapp_YoloV8Ncnn_detectPose(JNIEnv* env, jobject obj, jbyteArray imageData, jint width, jint height, jobject assetManager) {

    jbyte *data = env->GetByteArrayElements(imageData, nullptr);
    cv::Mat img(height, width, CV_8UC1, data);


    std::vector<Pose> poses = inference->runInference(img);
...
int Inference::loadNcnnNetwork(AAssetManager* mgr, const char* modeltype , const int& modelInputShape, const float* meanVals, const float* normVals, bool useGpu)
{
    ...
    char parampath[256];
    char modelpath[256];
    sprintf(parampath, "model.ncnn.param", modeltype);
    sprintf(modelpath, "model.ncnn.bin", modeltype);
